...
class Upload {
    ...
    public function cropimage( $data, $path ) {
        ...
        if( $src_w > $src_h ){
            ...
            /*if( $dst_h < $data['height'] ){
                $dst_h = $data['height'];
                $dst_w = round( ( $data['height']*$src_w ) / $src_h );
            }*/
        }
        elseif ( $src_h > $src_w ){
            $dst_w = round( ( $data['height']*$src_w ) / $src_h );
        ...
        $yy = 0;
        if( $dst_h > $data['height']  ){
            $yy = ($dst_h - $data['height'])/2;
        }


        $src_x = isset($data['x']) ? $data['x']: 0;
        $src_y = isset($data['y']) ? $data['y']: 0;


        /*$dst = array(0,0);

        if( $dst_h > $data['height'] ){
            $dst[1] = ($dst_h - $data['height'])/2;
        }*/

        // print_r( array( 'w'=> $dst_w, 'h'=>$dst_h, 'x'=>$src_x, 'y' => $src_y, 'yy' => $yy ) ); die;
        // $src_y = 0;

        $src_image = $this->import($path);
        ...
        switch( $this->getType($path) ){
            case 'bmp': imagewbmp($dst_image, $path); break;
            case 'gif': imagegif($dst_image, $path); break;
            case 'jpg':  case 'jpeg': imagejpeg($dst_image, $path); break;
            case 'png': imagepng($dst_image, $path, 9); break;
        }
    }
    public function cropImg($dest, $data ) {
